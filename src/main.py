...
def main() -> None:
    ...
    plot_timeline(
        video=VideoPlot(
            file=video_file, volume=video_volume, duration=int(video_duration)
        ),
        audios=[
            AudioPlot(
                file=audio_tracks_raw[idx]["file"],
                volume=track["volume"],
                delay=track["delay"],
                duration=(
                    int(get_media_duration(track["file"]))
                    if get_media_duration(track["file"])
                    else 0
                ),
                repeat=track["repeat"] if track["repeat"] != 0 else 1,
            )
    ...
    audio_sources = collect_audio_sources(
        video_volume, video_has_audio, audio_tracks, video_duration
    )
